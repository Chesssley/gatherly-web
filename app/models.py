...
class User(db.Model):
    ...
    merchant_status = db.Column(db.String(20), nullable=True)
...
def ensure_user_account_schema():
    ...
    if rows and "is_merchant" not in existing_columns:
        statements.append('ALTER TABLE "user" ADD COLUMN is_merchant BOOLEAN DEFAULT 0 NOT NULL')
    if rows and "merchant_status" not in existing_columns:
        statements.append('ALTER TABLE "user" ADD COLUMN merchant_status VARCHAR(20)')
...
def ensure_activity_schema():
    ...
    if rows and "is_official" not in existing_columns:
        statements.append("ALTER TABLE activity ADD COLUMN is_official BOOLEAN DEFAULT 0 NOT NULL")
